...
struct bit_vector {
    ...
    void save_fp(FILE* fp){
        ...
        essentials::save_vec_fp(fp, m_bits);
    }

    void load_fp(FILE* fp){
        uint64_t err = fread(&m_size, sizeof(m_size), 1, fp);
        if(!err) std::cout<<"Read 0 bytes in bit_vec: load_fp. should not happen\n";
        essentials::load_vec_fp(fp, m_bits);
    }

protected:
    size_t m_size; // should change to uint64_t?
    std::vector<uint64_t> m_bits;
};
