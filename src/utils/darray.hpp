...
struct darray {
    ...
    void save_fp(FILE* fp){
        fwrite(&m_positions, sizeof(m_positions), 1, fp);
        essentials::save_vec_fp(fp, m_block_inventory);
        essentials::save_vec_fp(fp, m_subblock_inventory);
        essentials::save_vec_fp(fp, m_overflow_positions);
    }

    void load_fp(FILE* fp){
        uint64_t err = fread(&m_positions, sizeof(m_positions), 1, fp);
        if(!err) std::cout<<"Read 0 bytes in darray: load_fp. should not happen\n";
        essentials::load_vec_fp(fp, m_block_inventory);
        essentials::load_vec_fp(fp, m_subblock_inventory);
        essentials::load_vec_fp(fp, m_overflow_positions);
    }
