...
template <typename T>
void save_vec_fp(FILE* fp, std::vector<T> const& vec){
    uint64_t n = vec.size();
    fwrite(&n, sizeof(n), 1, fp);
    fwrite(&vec[0],sizeof(T), n, fp);
}

template <typename T>
void load_vec_fp(FILE* fp, std::vector<T> &vec){
    uint64_t n;
    uint64_t err = fread(&n, sizeof(n), 1, fp);
    vec.resize(n);
    err = fread(&vec[0], sizeof(T), n, fp);
    if(!err && n>0) std::cout<<"Read 0 bytes in load_vec_fp. should not happen\n";
}
